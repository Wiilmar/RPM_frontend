...
const LasPreguntas = () => {
  ...
  const filteredPQRS = () => {
    let filtered = pqrs;
    if (filtro !== "") {
      filtered = filtered.filter((pqr) => pqr.asuntopqrs === filtro);
    }
    if (filtroEstado !== "") {
      const estadoFilter = filtroEstado === "pendiente";
      filtered = filtered.filter((pqr) => pqr.estado !== estadoFilter);
    }
    return filtered;
  };
  ...
  return (
    <div className="container mx-auto px-5 py-8">
      <h1 className="text-3xl text-white font-bold mb-6 text-left">
        Gestión de PQRS
      </h1>
      ...
      <div className="mb-4 flex items-center">
        ...
        <select id="filterEstado" value={filtroEstado} onChange={handleFilterEstado} className="block w-48 pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md ml-auto">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr className="text-center">
              <th scope="col" className="px-6 py-3 text-xs font-bold text-black uppercase tracking-wider">Nombre</th>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-black uppercase tracking-wider">Correo</th>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-black uppercase tracking-wider">Tipo de PQRS</th>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-black uppercase tracking-wider">Asunto</th>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-black uppercase tracking-wider">Acciones</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredPQRS().map((pqr) => (
              <tr key={pqr._id} className={`hover:bg-gray-100 transition-colors duration-200 ${pqr.estado === false ? "bg-pendiente" : pqr.estado === true ? "bg-verdemelamuerde" : ""}`}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center">{pqr.nombreusuario}</td> 
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{pqr.emailusuario}</td> 
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{pqr.asuntopqrs}</td>
                <td className="px-6 py-4 whitespace-normal text-sm text-gray-500 max-w-sm truncate text-left">{pqr.textopqrs}</td> 
